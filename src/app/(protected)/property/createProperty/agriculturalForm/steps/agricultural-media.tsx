...
interface AgriculturalMediaProps {
...
export const AgriculturalMedia: React.FC<AgriculturalMediaProps> = ({
  form,
  setUploading,
  uploading,
}) => {
                    ...
                    <div className="relative group cursor-pointer flex flex-col items-center justify-center w-full max-w-md h-32 rounded-xl border-2 border-dashed border-muted-foreground/25 hover:border-primary/50 hover:bg-primary/5 transition-all duration-200">
                      <div className="flex flex-col items-center justify-center gap-3 text-muted-foreground group-hover:text-primary transition-colors">
                        ...
                        <div className="text-center space-y-1.5">
                          ...
                          <p className="text-xs text-muted-foreground/75">
                            Add multiple photos to showcase the land
                          </p>
                        </div>
                      </div>
                      <input
                        ...
                        {field.value?.map((doc: { url: string; name?: string }, index: number) => (
                          ...
                          >
                            <Image
                              ...
                              height={200}
                              src={doc.url}
                              alt={doc.name || `Land layout ${index + 1}`}
                              className="object-cover w-full h-full transition-transform duration-300 group-hover:scale-105"
                            />
                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                              <Button
                      ...
                      <div className="space-y-2">
                        {field.value?.map((doc: { url: string; name?: string }, index: number) => (
                          <div key={index} className="flex items-center gap-2">
                            ...
                            <Input
                              placeholder={`Document ${index + 1} name (e.g. Layout Plan, Site Map)`}
                              value={doc.name || ""}
                              onChange={(e) => {
