...
import { Bell, Check, Clock, X, Building2, Home, Timer, UserCircle, Loader2, XCircle, Trash2, AlertCircle } from "lucide-react";
...
export const Notifications = () => {
  ...
  const handleDismissExpiredRequest = (requestId: string) => {
    setDismissedRequestIds((prev) => new Set(prev).add(requestId));
  };

  const visibleContactRequests = useMemo(
    () => pendingContactRequests?.filter((r) => !dismissedRequestIds.has(r._id)) ?? [],
    [pendingContactRequests, dismissedRequestIds]
  );
  ...
  const hasContent =
    ...
    (visibleContactRequests.length > 0);
  ...
  const InvitationItem = ({
    ...
    <Card className="mb-2 sm:mb-3 border-blue-200 bg-blue-50/50 dark:bg-primary/10 transition-all duration-200 overflow-hidden">
  ...
  const ContactRequestItem = ({
    ...
    const timeLeft = isExpired ? null : formatDistanceToNow(expiresAt, { addSuffix: false });
    const isExpiringSoon = !isExpired && expiresAt.getTime() - Date.now() < 12 * 60 * 60 * 1000;
    ...
    return (
      <Card className={cn(
        "mb-2 sm:mb-3 transition-all duration-200 overflow-hidden",
        isExpired
          ? "border-gray-200 bg-gray-50/50 dark:bg-gray-900/20 opacity-75"
        ...
        <CardContent className="p-3 sm:p-4">
            ...
            <div className="flex-1 min-w-0 space-y-1">
              <div className="flex items-center gap-2">
                <div className={cn(
                  ...
                  isExpired ? "bg-gray-400" : "bg-amber-500"
                )} />
              ...
              {isExpired ? (
                <div className="flex items-center gap-1 text-[10px] sm:text-xs text-red-600 dark:text-red-400">
                ...
                <div className={cn(
                  ...
                  <Timer className="h-2.5 w-2.5 sm:h-3 sm:w-3 flex-shrink-0" />
          ...
          <div className="flex gap-2 mt-2 sm:mt-3 justify-end">
            {isExpired ? (
              <Button
                onClick={() => handleDismissExpiredRequest(request._id)}
                variant="outline"
                size="sm"
                className="h-6 px-2 text-[10px] sm:h-7 sm:px-3 sm:text-xs text-gray-600 hover:text-gray-800 hover:bg-gray-100"
              >
  ...
  const NotificationItem = ({
    ...
    <Card
      ...
      className={`mb-2 sm:mb-3 transition-all duration-200 overflow-hidden ${isUnread
        ...
        <div className="flex items-start justify-between gap-2 sm:gap-3 min-w-0">
          <div className="flex-1 min-w-0 space-y-0.5 sm:space-y-1">
            <div className="flex items-start gap-2">
              {isUnread && (
                <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-blue-500 rounded-full flex-shrink-0 mt-1 dark:bg-blue-500" />
              )}
              <h4
                className={`text-xs sm:text-sm font-medium min-w-0 truncate flex-1 ${isUnread
  ...
  return (
    <>
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        ...
        <SheetContent className="max-w-[92vw] p-3 sm:p-6 pt-[8vh] md:pt-none overflow-x-hidden overflow-y-auto my-auto rounded-l-2xl md:min-w-[30vw]">
          ...
          <ScrollArea className="h-[calc(100vh-100px)] pr-1 sm:pr-4">
            {isLoading ? (
                ...
                {visibleContactRequests.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-2 sm:mb-4">
                      ...
                      >
                        {visibleContactRequests.length}
                      </Badge>
                    </div>
                    {visibleContactRequests.map((request) => (
                      <ContactRequestItem
                        key={request._id}
                        request={request}
                      />
